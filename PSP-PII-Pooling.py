...
def read_data(filePath, typeNucle):
    '''
    :param filePath: the path of data
    :param typeNucle: DNA or RNA
...
def save_result(filePath, alpha, beta):
    '''
    :param filePath: the path of data
    :param alpha: the distance of between first nucleotide and second nucleotide
    :param beta: the distance of between second nucleotide and third nucleotide
    ...
    seqList, label, lenSeq = read_data(filePath, nucleotide)
    ...
    if alpha == 0 and beta == 0:
        ...
        if alpha + beta > math.floor((lenSeq - 5) / 2):
            ...
            sys.exit()
    dataVectorPSPPII = []
    iterate = 20  # 轮数，从1开始，最终值为样本总数除num值的向上取整
    num = 850  # 每轮样本数
    for seqID in range(num*(iterate-1), num*iterate):
        print(seqID)
        if seqID == len(seqList): # 防止最后一次越界
            break
        seqLine = seqList[seqID]
    ...
    np.savetxt("./result/data_" + dataName + "_PSP_PII_alpha_" + str(alpha) + "_beta_" 
               + str(beta) + "_iterate_" + str(iterate) + ".csv", dataVectorPSPPII, delimiter=" ")
...
if __name__ == '__main__':
    ...
    beta = 0
    if nucleotide == 'RNA':
        # fPath = './data/RNA_m6A/non-single-base'
        # fPath = './data/RNA_m6A/single-base'
        # for i, j, k in os.walk(fPath):
        #     for fName in k:
        #         print(fName)
        #         file_path = fPath + '/' + fName
        #         save_result(file_path, alpha, beta)
        # 单个数据
        file_path = './data/RNA_m6A/non-single-base/Homo.fasta'
        save_result(file_path, alpha, beta)
    elif nucleotide == 'DNA':
        fPath = './data/DNA_4mC'
        for i, j, k in os.walk(fPath):
